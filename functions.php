...
/**
 *
 * Functions to add the configuration changes to the UCF RSS plugin
 *
 */
...
if ( !class_exists( 'UCF_RSS_Option' ) ) {
	...
	class UCF_RSS_Option {
		public
			...
			$hr	= null; // Adding option for the HR layout

		function __construct( $option_name, $args=array() ) {
			$this->hr         = isset( $args['hr'] ) ? $args['hr'] : $this->hr;
		}

		/**
		 * Returns the hr value for the option, with the Options API value
		 ...
		 **/
		function get_hr( $apply_configurable_val=true ) {
			$hr = $this->hr;
			if ( $this->options_page && $apply_configurable_val ) {
				$hr = get_option( UCF_RSS_Config::$option_prefix . $this->option_name, $hr );
			}
			return $hr;
		}
